...
class quicksand:
    ...
    def readDir(directory, capture=False, strings=True, debug=False, timeout=0, exploityara=None, execyara=None,
                pdfyara=None, password=None):
        out = {}
        for f in listdir(directory):
            fname = join(directory, f)
            if isfile(fname):
                # print (fname)
                q = quicksand(fname, capture=capture, strings=strings, debug=debug, timeout=timeout,
                              exploityara=exploityara, execyara=execyara, pdfyara=pdfyara, password=password)
                q.process()
                out[fname] = q.results
        return out
    ...
    def __init__(self, data, capture=False, strings=True, debug=False, timeout=0, exploityara=None, execyara=None,
                 pdfyara=None, password=None, filename=None):
        ...
        if matches:
            for m in matches:
                ...
                if loc in self.results['results']:
                    if self.strings:
                        self.results['results'][loc].append(
                            {'rule': m.rule, 'desc': desc, 'strings': m.strings, 'type': rtype, 'mitre': mitre})
                    else:
                        self.results['results'][loc].append(
                            {'rule': m.rule, 'desc': desc, 'type': rtype, 'mitre': mitre})
                else:
                    if self.strings:
                        self.results['results'][loc] = [
                            {'rule': m.rule, 'desc': desc, 'strings': m.strings, 'type': rtype, 'mitre': mitre}]
                    else:
    ...
    def analyse_pdf(self, doc, loc):
        try:
            ...
            for block in re.findall(
                    b'((\x0a|\x0d|\x20)(\d{1,4})[^\d]{1,3}(\d{1,2})\sobj|(\x0a|\x0d)(xref|trailer)(\x0a|\x0d))', doc):
                if len(block[2]) != 0:
                    num = int(block[2])
                    gen = int(block[3])
            ...
            for block in re.findall(
                    b'((\x0a|\x0d|\x20)(\d{1,4})[^\d]{1,3}(\d{1,2})\sobj(\x0a|\x0d|\x20)<<[^>]{1,200}\x2fFilter)', doc):
            ...
            if loc in self.results['results']:
                self.results['results'][loc].append(
                    {'rule': "pdf_malformed", 'desc': "WARNING: PDF is malformed", 'strings': '', 'type': 'structure'})
            else:
                self.results['results'][loc] = [
                    {'rule': "pdf_malformed", 'desc': "WARNING: PDF is malformed", 'strings': '', 'type': 'structure'}]
    ...
    def rtf_block(self, block, loc):
        ...
        if re.search(binascii.unhexlify(b'd0cf11e0'), obj, re.IGNORECASE):
            ...
            for i in range(0, len(elements)):
                ...
                if "negative" in str(r) and len(elements) > 1:
                    ...
                    r = quicksand.analyse_ole(self, b''.join(elements[directory: i + 1]),
                                              str(loc) + "-oleg" + str(directory) + "-" + str(i))

                    directory = i + 1
                    if self.capture:
                        self.results['streams'][str(loc) + "-oler" + str(directory) + "-" + str(i)] = b''.join(
                            elements[directory: i + 1])
                    else:
                        if self.capture:
        ...
        if re.search(binascii.unhexlify(b'504B030414000600'), obj, re.IGNORECASE):
            ...
            for i in range(0, len(elements)):
                ...
                if "negative" in str(r) and len(elements) > 1:
                    quicksand.msg(self, "range " + str(directory) + " " + str(i))
                    r = quicksand.analyse_openxml(self, b''.join(elements[directory: i + 1]),
                                                  str(loc) + "-openxmlg" + str(directory) + "-" + str(i))
                    directory = i + 1
                    if self.capture:
                        self.results['streams'][str(loc) + "-openxmlr" + str(directory) + "-" + str(i)] = b''.join(
                            elements[directory: i + 1])
                else:
                    if self.capture:
        ...
        try:
            ...
            for obj in rt.objects:
                ...
                if obj.is_package:
                    ...
                    self.results['packages'].append(
                        {'loc': obj.start, 'filename': obj.filename, 'src_path': obj.src_path,
                         'temp_path': obj.temp_path})

                if obj.clsid is not None:
                    quicksand.msg(self, obj.clsid)
                    quicksand.msg(self, obj.clsid_desc)
                    quicksand.scan_exploit(self, obj.rawdata,
                                           str(loc) + "-rtfcls@" + str(obj.start) + "_" + str(obj.clsid))
        ...
        try:
            ...
            if meta.bytes != None:
                ...
                if extra > 1000:
                    ...
                    if loc in self.results['results']:
                        self.results['results'][loc].append({'rule': "olefile_end",
                                                             'desc': "WARNING: outside end of OLE structure has " + str(
                                                                 extra) + " bytes", 'strings': '', 'type': 'structure'})
                    else:
                        self.results['results'][loc] = [{'rule': "olefile_end",
                                                         'desc': "WARNING: outside end of OLE structure has " + str(
                                                             extra) + " bytes", 'strings': '', 'type': 'structure'}]
            ...
            for name in ole.listdir(streams=True, storages=False):
                ...
                try:
                    ...
                    if re.search(binascii.unhexlify(b'504B030414000600'), s, re.IGNORECASE):
                        ...
                        directory = 0
                        for i in range(0, len(elements)):
                            r = quicksand.analyse_openxml(self, elements[i],
                                                          str(loc) + "-" + str(name0) + "-" + "openxml" + str(i))
                            if "negative" in str(r):
                                r = quicksand.analyse_openxml(self, b''.join(elements[directory: i + 1]),
                                                              str(loc) + "-" + str(name0) + "-" + "openxml" + str(
                                                                  directory) + "-" + str(i))
                                directory = i + 1
                                if self.capture:
                                    self.results['streams'][
                                        str(loc) + "-" + str(name0) + "-" + "openxml" + str(directory) + "-" + str(
                                            i)] = b''.join(elements[directory: i + 1])
                            else:
                                if self.capture:
                                    self.results['streams'][str(loc) + "-" + str(name0) + "-" + "openxml" + str(i)] = \
                                    elements[i]
        ...
        if re.search(binascii.unhexlify(b'504B030414000600'), doc, re.IGNORECASE):
            ...
            for i in range(0, len(elements)):
                ...
                if "negative" in str(r):
                    # quicksand.msg (self,"range " + str(directory) + " " + str(i))
                    r = quicksand.analyse_openxml(self, b''.join(elements[directory: i + 1]),
                                                  str(loc) + "openxml-" + str(directory) + "-" + str(i))
                    directory = i + 1
                    if self.capture:
                        self.results['streams'][str(loc) + "openxml-" + str(directory) + "-" + str(i)] = b''.join(
                            elements[directory: i + 1])
                else:
                    if self.capture:
